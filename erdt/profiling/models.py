...
class Grant(PolymorphicModel):
    ...
    def grant_financial_release_link(self):
        if self.id:
            url = '/grant_financial_releases/%d' % self.id
            return format_html(u'<a href="{}">%s</a>' % 'Click to download financial report.', url)
        return ''
    grant_financial_release_link.short_description = 'Grant financial report'
